...
TSIN_OLD_DIR       = "/etc/tor/instances"
...
def show_banner():
 ...
 ███████╗███████╗██╗██║     ██║  ██║██║ ╚████║███████╗███████╗
 ╚══════╝╚══════╝╚═╝╚═╝     ╚═╝  ╚═╝╚═╝  ╚═══╝╚══════╝╚══════╝{C_RESET}""")
    print(f"{C_BOLD}{C_GREEN}                      EZ-PANEL AUTOMATOR                     {C_RESET}")
    print(f"{C_YELLOW}             Version: V1.4  {C_WHITE}|  {C_CYAN}Dev : t.me/T_Sinn{C_RESET}")
    print(f"{C_MAGENTA}=" * 66 + C_RESET)
    print()
...
def get_installed_tsin_nodes():
    installed = {}
    for d in [TSIN_INSTANCES_DIR, TSIN_OLD_DIR]:
        if os.path.exists(d):
            try:
                for item in os.listdir(d):
                    if item.startswith("tsin_"):
                        code = item.replace("tsin_", "")
                        for c in COUNTRIES:
                            if c['code'] == code:
                                installed[code] = c.copy()
                                break
            except Exception:
                pass
    return installed
...
def check_node_configured_in_panel(client, country_code):
    try:
        ...
        xray_setting = settings.get("xraySetting", "{}")
        xray_config  = json.loads(xray_setting) if isinstance(xray_setting, str) else xray_setting
        outbound_tag = f"T.Sin-{country_code.upper()}"
...
def clone_inbound_for_country(client, source_inbound, country):
    ...
    outbound_tag = f"T.Sin-{country['code'].upper()}"  
...
def handle_tsin_nodes_setup(client):
    ...
    if not installed:
        show_banner()
        print(f"{C_YELLOW}📦 No T.Sin nodes installed on your system.{C_RESET}")
        input(f"\nPress {C_BOLD}{C_WHITE}[Enter]{C_RESET} to return...")
        return

    unconfigured = sorted(
        [info for code, info in installed.items() if not check_node_configured_in_panel(client, code)],
        key=lambda x: x['out_port']
    )

    if not unconfigured:
        show_banner()
        print(f"{C_GREEN}🎉 All installed T.Sin nodes are already configured!{C_RESET}")
        input(f"\nPress {C_BOLD}{C_WHITE}[Enter]{C_RESET} to return...")
        return
    ...
    while True:
        ...
        show_banner()
        print(f"{C_RED}📌{C_RESET} {C_BOLD}{C_CYAN}[ T.SIN — SELECT NODES TO SETUP ]{C_RESET}")
        print(f"{C_BLUE}{'─' * 66}{C_RESET}")
        for i, node in enumerate(unconfigured, 1):
            print(f"  {C_YELLOW}[{i:02}]{C_RESET} {node.get('flag', '🌐')} {C_WHITE}{node['name']:<20}{C_RESET}  SOCKS Port: {C_CYAN}{node['out_port']}{C_RESET}")
        print(f"{C_BLUE}{'─' * 66}{C_RESET}")
        ...
        show_banner()
        print(f"{C_RED}📌{C_RESET} {C_BOLD}{C_CYAN}[ T.SIN — SELECT INBOUND SOURCE ]{C_RESET}")
...
def handle_3xui_flow():
    ...
    while True:
            ...
            while True:
                ...
                if del_choice == "1":
                    ...
                    while True:
                        ...
                        show_banner()
                        print(f"{C_RED}📌{C_RESET} {C_BOLD}{C_CYAN}[ SELECT COUNTRIES TO DELETE ]{C_RESET}")
                        print(f"{C_BLUE}{'─' * 66}{C_RESET}")
        ...
        if choice == "1":
            ...
            show_banner()
            print(f"{C_RED}📌{C_RESET} {C_BOLD}{C_CYAN}[ BATCH CREATE — SELECT INBOUND SOURCE ]{C_RESET}")
            xui_show_inbounds(inbounds)
            print(f"  {C_RED}[0]{C_RESET} Go Back")
            print(f"{C_BLUE}{'─' * 62}{C_RESET}")
            while True:
            ...
            while True:
                ...
                show_banner()
                print(f"{C_RED}📌{C_RESET} {C_BOLD}{C_CYAN}[ CUSTOM CREATE — SELECT INBOUND SOURCE ]{C_RESET}")
                xui_show_inbounds(inbounds)
                print(f"  {C_RED}[0]{C_RESET} Go Back")
                print(f"{C_BLUE}{'─' * 62}{C_RESET}")
                while True:
...
def pg_show_inbounds(inbounds):
    ...
    for i, ib in enumerate(inbounds, 1):
        ...
        print(f"  {C_YELLOW}[{i}]{C_RESET}  {C_WHITE}{port:<8}{C_RESET}{C_CYAN}{protocol:<12}{C_RESET}"
              f"{C_MAGENTA}{network:<10}{C_RESET}{C_GREEN}{security:<10}{C_RESET}{tag}")
    print(f"{C_BLUE}{'─' * 72}{C_RESET}")
...
def pg_pick_inbound(api: PasarGuardAPI):
    ...
    if not inbounds:
        ...
        return None
    print(f"\n{C_RED}📌{C_RESET} {C_BOLD}{C_CYAN}[ SELECT INBOUND TO CLONE FROM ]{C_RESET}")
...
def pg_pick_host(api: PasarGuardAPI):
    ...
    print(f"\n{C_RED}📌{C_RESET} {C_BOLD}{C_CYAN}[ SELECT HOST TO CLONE FROM ]{C_RESET}")
...
def handle_pg_tsin_nodes(api: PasarGuardAPI):
    ...
    if not installed:
        show_banner()
        print(f"{C_YELLOW}📦 No T.Sin nodes installed on this system.{C_RESET}")
        input(f"\nPress {C_BOLD}{C_WHITE}[Enter]{C_RESET} to return...")
        return
...
def main():
    ...
    if len(installed_list) == 1:
        ...
        show_banner()
        print(f"{C_CYAN}Thank you for using EZ-Panel. Goodbye!{C_RESET}\n")
        sys.exit(0)


    while True:
        show_banner_with_panels()
        print(f"{C_RED}📌{C_RESET} {C_BOLD}{C_CYAN}[ SELECT SYSTEM PANEL MODULE ]{C_RESET}")
